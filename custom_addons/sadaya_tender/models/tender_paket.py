...
class TenderPaket(models.Model):
    _name = 'tender.paket'
    _description = 'Paket Pengadaan/Tender'
    ...
    jadwal_ids = fields.One2many('tender.jadwal', 'paket_id', string='Jadwal Tender')
    dokumen_ids = fields.One2many('tender.dokumen', 'paket_id', string='Dokumen Pemilihan')
    penawaran_ids = fields.One2many('tender.penawaran', 'paket_id', string='Dokumen Penawaran')

    @api.model_create_multi
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('kode_tender', 'New') == 'New':
                vals['kode_tender'] = self.env['ir.sequence'].next_by_code('tender.paket') or 'New'
