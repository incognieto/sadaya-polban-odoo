...
class SadayaRancangPortal(http.Controller):
    ...
    @http.route(['/sadaya_rancang'], type='http', auth="public", website=True)
    def index_rancang(self, **kwargs):
        return request.render('sadaya_rancang.portal_rancang_index')

    # Route: Daftar Pengajuan Kebutuhan
    @http.route(['/sadaya_rancang/pengajuan'], type='http', auth="public", website=True)
    def list_pengajuan(self, **kwargs):
        usulans = request.env['rancang.usulan'].sudo().search([])
        return request.render('sadaya_rancang.portal_rancang_pengajuan', {'usulans': usulans})

    # Route: Form Buat Pengajuan Baru
    @http.route(['/sadaya_rancang/pengajuan/baru'], type='http', auth="public", website=True, methods=['GET', 'POST'])
    def form_baru_pengajuan(self, **kwargs):
        if request.httprequest.method == 'POST':
            ...
            return request.redirect('/sadaya_rancang/pengajuan')

        return request.render('sadaya_rancang.portal_rancang_form')

    # Route: Detail Pengajuan
    @http.route(['/sadaya_rancang/pengajuan/<model("rancang.usulan"):usulan>'], type='http', auth="public", website=True)
    def detail_pengajuan(self, usulan, **kwargs):
        return request.render('sadaya_rancang.portal_rancang_detail', {'usulan': usulan})

    # Route: Revert Pengajuan (Kembali ke Draft)
    @http.route(['/sadaya_rancang/pengajuan/<model("rancang.usulan"):usulan>/revert'], type='http', auth="public", website=True)
    def revert_pengajuan(self, usulan, **kwargs):
        if usulan.state in ['submitted', 'returned', 'rejected']:
            usulan.sudo().action_draft()
        return request.redirect(f'/sadaya_rancang/pengajuan/{usulan.id}')

    # Route: Hapus Pengajuan
    @http.route(['/sadaya_rancang/pengajuan/<model("rancang.usulan"):usulan>/delete'], type='http', auth="public", website=True)
    ...
    @http.route(['/sadaya_rancang/pengajuan/<model("rancang.usulan"):usulan>/submit'], type='http', auth="public", website=True)
    ...
    @http.route(['/sadaya_rancang/pengajuan/<model("rancang.usulan"):usulan>/approve'], type='http', auth="public", website=True)
    ...
    @http.route(['/sadaya_rancang/pengajuan/<model("rancang.usulan"):usulan>/reject'], type='http', auth="public", website=True, methods=['POST'])
    ...
    @http.route(['/sadaya_rancang/pengajuan/<model("rancang.usulan"):usulan>/publish'], type='http', auth="public", website=True)
    ...
    @http.route(['/sadaya_rancang/persetujuan_manajemen'], type='http', auth="public", website=True)
    def list_persetujuan(self, **kwargs):
        usulans = request.env['rancang.usulan'].sudo().search([('state', '=', 'submitted')])
        return request.render('sadaya_rancang.portal_rancang_persetujuan', {'usulans': usulans})
